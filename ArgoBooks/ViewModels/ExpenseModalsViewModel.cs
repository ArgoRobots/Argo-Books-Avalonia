...
public partial class ExpenseModalsViewModel : TransactionModalsViewModelBase<ExpenseDisplayItem, ExpenseLineItem>
{
    ...
    private void AddSupplierFromName(string? name)
    {
        ...
        var option = new CounterpartyOption { Id = newId, Name = newSupplier.Name };
        SupplierOptions.Add(option);
        SelectedSupplier = option;

        var action = new DelegateAction(
            $"Create supplier '{name.Trim()}'",
            () =>
            {
                companyData.Suppliers.Remove(newSupplier);
                var toRemove = SupplierOptions.FirstOrDefault(o => o.Id == newId);
                if (toRemove != null) SupplierOptions.Remove(toRemove);
                if (SelectedSupplier?.Id == newId)
                    SelectedSupplier = null;
            },
            () =>
            {
                companyData.Suppliers.Add(newSupplier);
                var opt = new CounterpartyOption { Id = newId, Name = newSupplier.Name };
                SupplierOptions.Add(opt);
                SelectedSupplier = opt;
            });
        App.UndoRedoManager.RecordAction(action);
    ...
    /// <summary>
    /// Creates a new expense category from the name typed into the category SearchableDropdown's "Add new" button,
    /// adds it to data and the options list, and selects it.
    /// </summary>
    [RelayCommand]
    private void AddCategoryFromName(string? name)
    {
        if (string.IsNullOrWhiteSpace(name)) return;
        ...
        var action = new DelegateAction(
            $"Create category '{name.Trim()}'",
            () =>
            {
                companyData.Categories.Remove(newCategory);
                var toRemove = CategoryOptions.FirstOrDefault(o => o.Id == newId);
                if (toRemove != null) CategoryOptions.Remove(toRemove);
                if (SelectedCategory?.Id == newId)
                    SelectedCategory = null;
            },
            () =>
            {
                companyData.Categories.Add(newCategory);
                var opt = new CategoryOption { Id = newId, Name = newCategory.Name };
    ...
    /// <summary>
    /// Creates a new expense product from the name typed into a line item's product SearchableDropdown's
    /// "Add new" button, adds it to data and ProductOptions, and selects it on that line item.
    /// Called via per-line-item closure command set in <see cref="OnLineItemAdded"/>.
    /// </summary>
    private void AddProductFromNameForLineItem(string? name, ExpenseLineItem lineItem)
    {
        ...
        var capturedProduct = newProduct;
        var action = new DelegateAction(
            $"Create product '{name.Trim()}'",
            () =>
            {
                companyData.Products.Remove(capturedProduct);
                var toRemove = ProductOptions.FirstOrDefault(o => o.Id == newId);
                if (toRemove != null) ProductOptions.Remove(toRemove);
                if (lineItem.SelectedProduct?.Id == newId)
                    lineItem.SelectedProduct = null;
            },
            () =>
            {
                companyData.Products.Add(capturedProduct);
                ProductOptions.Add(capturedOption);
                lineItem.SelectedProduct = capturedOption;
            });
        App.UndoRedoManager.RecordAction(action);
    ...
    /// <summary>
    /// Wires the per-line-item AddProductFromNameCommand closure on each new line item.
    /// </summary>
    protected override void OnLineItemAdded(ExpenseLineItem lineItem)
    {
...
public class ExpenseLineItem : TransactionLineItemBase
{
    ...
    /// Receives the typed search text and creates a new product, then selects it on this line item.
    /// Set by the parent <see cref="ExpenseModalsViewModel"/> via <see cref="ExpenseModalsViewModel.OnLineItemAdded"/>.
    /// </summary>
