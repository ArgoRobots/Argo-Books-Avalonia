...
public partial class StatCardWidgetViewModel : WidgetViewModelBase
{
    ...
    private void LoadRevenue(CompanyData data, DateTime startDate, DateTime endDate)
    {
        ...
        var (prevStart, prevEnd) = GetComparisonPeriod();
        if (prevStart != DateTime.MinValue && HasSufficientPriorData(data, prevStart))
        {
            var prevUSD = data.Revenues
                .Where(s => s.Date >= prevStart && s.Date <= prevEnd)
                .Sum(s => s.EffectiveSubtotalUSD);
            ChangeValue = CalculatePercentageChange(prevUSD, currentUSD);
            ChangeText = FormatPercentageChange(ChangeValue);
        }
        else
        {
            ChangeValue = null;
    ...
    private static (DateTime prevStart, DateTime prevEnd) GetComparisonPeriod()
    {
        var chartSettings = ChartSettingsService.Instance;
